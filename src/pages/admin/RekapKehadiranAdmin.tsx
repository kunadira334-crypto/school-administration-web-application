import { useMemo, useState } from 'react';
import { db } from '../../lib/storage';
import { AttendanceStatus, CLASS_LIST } from '../../types';
import { Badge, Card, EmptyRow, Field, inputCls, PageTitle, Table } from '../../components/ui';

export default function RekapKehadiranAdmin() {
  const attendance = db.getAttendance();
  const [kelas, setKelas] = useState('');
  const [dari, setDari] = useState('');
  const [sampai, setSampai] = useState('');
  const [status, setStatus] = useState('');

  const filtered = useMemo(() => {
    return attendance.filter((a) => {
      if (kelas && a.kelas !== kelas) return false;
      if (dari && a.tanggal < dari) return false;
      if (sampai && a.tanggal > sampai) return false;
      if (status && a.status !== status) return false;
      return true;
    });
  }, [attendance, kelas, dari, sampai, status]);

  const recap = useMemo(() => {
    const map: Record<string, { siswaId: string; nama: string; kelas: string; Hadir: number; Izin: number; Sakit: number; Alpha: number }> = {};
    filtered.forEach((a) => {
      if (!map[a.siswaId]) map[a.siswaId] = { siswaId: a.siswaId, nama: a.nama, kelas: a.kelas, Hadir: 0, Izin: 0, Sakit: 0, Alpha: 0 };
      map[a.siswaId][a.status as AttendanceStatus]++;
    });
    return Object.values(map);
  }, [filtered]);

  return (
    <div>
      <PageTitle title="Rekap Kehadiran" subtitle="Filter berdasarkan tanggal, kelas, dan status" />
      <Card className="mb-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <Field label="Kelas">
            <select className={inputCls} value={kelas} onChange={(e) => setKelas(e.target.value)}>
              <option value="">Semua</option>
              {CLASS_LIST.map((k) => (
                <option key={k} value={k}>
                  Kelas {k}
                </option>
              ))}
            </select>
          </Field>
          <Field label="Dari Tanggal">
            <input type="date" className={inputCls} value={dari} onChange={(e) => setDari(e.target.value)} />
          </Field>
          <Field label="Sampai Tanggal">
            <input type="date" className={inputCls} value={sampai} onChange={(e) => setSampai(e.target.value)} />
          </Field>
          <Field label="Status">
            <select className={inputCls} value={status} onChange={(e) => setStatus(e.target.value)}>
              <option value="">Semua</option>
              <option>Hadir</option>
              <option>Izin</option>
              <option>Sakit</option>
              <option>Alpha</option>
            </select>
          </Field>
        </div>
      </Card>

      <Card className="mb-4">
        <h3 className="font-bold text-gray-700 mb-2">Rekap per Siswa</h3>
        <Table headers={['Nama', 'Kelas', 'Hadir', 'Izin', 'Sakit', 'Alpha']}>
          {recap.length === 0 && <EmptyRow colSpan={6} />}
          {recap.map((r) => (
            <tr key={r.siswaId}>
              <td className="px-3 py-2 font-medium">{r.nama}</td>
              <td className="px-3 py-2">{r.kelas}</td>
              <td className="px-3 py-2">{r.Hadir}</td>
              <td className="px-3 py-2">{r.Izin}</td>
              <td className="px-3 py-2">{r.Sakit}</td>
              <td className="px-3 py-2">{r.Alpha}</td>
            </tr>
          ))}
        </Table>
      </Card>

      <Card>
        <h3 className="font-bold text-gray-700 mb-2">Detail Kehadiran</h3>
        <Table headers={['Tanggal', 'Kelas', 'Nama', 'Status', 'Catatan', 'Input Oleh']}>
          {filtered.length === 0 && <EmptyRow colSpan={6} />}
          {filtered.map((a) => (
            <tr key={a.id}>
              <td className="px-3 py-2">{a.tanggal}</td>
              <td className="px-3 py-2">{a.kelas}</td>
              <td className="px-3 py-2">{a.nama}</td>
              <td className="px-3 py-2">
                <Badge status={a.status} />
              </td>
              <td className="px-3 py-2">{a.catatan || '-'}</td>
              <td className="px-3 py-2">{a.inputOleh}</td>
            </tr>
          ))}
        </Table>
      </Card>
    </div>
  );
}
